defmodule EstimateWeb.Components.SearchComponent do
    ...
    <div class="relative" id={@id} phx-hook="SearchFocus" phx-target={@myself}>
      <div class="relative">
        ...
        <input
          ...
          autocomplete="off"
          class="w-full pl-9 pr-16 py-1.5 text-sm bg-base-200 border border-base-300 rounded-lg text-base-content placeholder:text-base-content/40"
        />
        <div class="absolute right-3 top-1/2 -translate-y-1/2 flex items-center gap-1">
          ...
          <kbd
            :if={@query == ""}
            class="hidden sm:inline-flex items-center px-1.5 py-0.5 text-xs text-base-content/40 bg-base-300 rounded"
          >
            /
          </kbd>
        </div>
      ...
      <div
        ...
        id={"#{@id}-dropdown"}
        class="absolute top-full left-0 right-0 mt-1 bg-base-100 border border-base-300 rounded-lg shadow-lg overflow-hidden z-50"
        ...
        <div :if={@results != []} class="max-h-80 overflow-y-auto">
          <%= for {result, idx} <- Enum.with_index(@results) do %>
            <.link
              ...
              class={[
                "flex items-center gap-3 px-4 py-2.5 hover:bg-base-200 transition-colors",
                idx == @selected_index && "bg-base-200"
              ]}
            ...
            >
              ...
              <div class="flex-1 min-w-0">
                ...
                <p :if={result.subtitle} class="text-xs text-base-content/60 truncate">
                  {result.subtitle}
                </p>
              </div>
            </.link>
  ...
  defp type_badge_class("customer"), do: "bg-info/10 text-info"
  defp type_badge_class("project"), do: "bg-success/10 text-success"
  defp type_badge_class("estimation"), do: "bg-warning/10 text-warning"
end
