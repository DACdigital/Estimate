...
- **Enhance in place** — task and epic modals grow a ✨ button when AI is configured. The call runs in a background `Task` (the UI never blocks — a spinner does), and the result is written back into the textarea through a `push_event`, form-change semantics intact.
...
| `DATABASE_ROLE_PASSWORD` | migrations | no | Password for the `estimate_app` RLS role (default `estimate_app_dev`) |
