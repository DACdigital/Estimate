...
| `DATABASE_ROLE_PASSWORD` | migrations | no | **No longer usable for connecting.** `estimate_app` is `NOLOGIN` (see the `HardenAppRole` migration) and is only ever reached via `SET ROLE` on a connection already authenticated as the `DATABASE_URL` role — this var only sets a password on the (now unloginable) role at `CREATE ROLE` time. `bin/migrate` must always run as the `DATABASE_URL` role — a **superuser** account (required for `CREATE ROLE ... BYPASSRLS`, used by both the `HardenAppRole` and `CreateEstimateSystemRole` migrations) — never as `estimate_app`; that's what `SKIP_RLS_ROLE=true` in `migrate_and_server` ensures. If the `DATABASE_URL` role ever changes, re-run `GRANT estimate_system TO <new_role>` (grants for `estimate_app` are unaffected) — otherwise every `Repo.without_rls/1` call raises. Before deploying, confirm the migrating role is a superuser: `psql "$DATABASE_URL" -tAc "select rolsuper from pg_roles where rolname = current_user"` must print `t`. |
...
- **Migrations need a superuser** — `CREATE ROLE ... BYPASSRLS` (the `HardenAppRole` and `CreateEstimateSystemRole` migrations) can only be run by a superuser. Pre-deploy check: `psql "$DATABASE_URL" -tAc "select rolsuper from pg_roles where rolname = current_user"` must print `t`. If the `DATABASE_URL` role is ever rotated to a different role, that new role needs `estimate_system` re-granted (`GRANT estimate_system TO <new_role>`; `estimate_app` grants are separate and unaffected) or `Repo.without_rls/1` starts raising.
