defmodule EstimateWeb.ProjectLive.ShowTest do
  ...
  describe "authorization denial resets the pending confirm-state (behavior-identical guard)" do
    ...
    # (Show.Authz.gate/4's deny_assigns). These 2 pin it: a viewer sets the confirm-state,
    ...
    #
    # NOTE: this block used to include a 3rd case for delete_project. confirm_delete_project is
    # now itself gated by require_can_delete/3 (Task 3, security audit), so a viewer can no
    # longer diverge deleting_project from its mount default of false at all -- any replacement
    # test asserting it stays false on a direct delete_project call would be tautological (can't
    # tell "the gate resets it" from "the gate never touches it"). That gap is covered instead by
    # "delete_project is gated for a viewer even with a matching confirmation name" above and
    # "viewer cannot open the delete-project confirmation" in the "danger zone / delete project"
    # describe block, both of which diverge a real assign first.
    setup :setup_project
